...
validateUtils <- function(formula,
    ...
    if (type == "linear") {
        ...
        tryCatch(
            expr = {
                glm(formula, data = data, family = type_glm, control = glm.control(epsilon = 1e-8, maxit = 100, trace = FALSE))
            },
            error = function(e) {
                print(paste("There was an error message."))
            },
            warning = function(w) {
                if (w$message %in% c("glm.fit: algorithm did not converge")) {
                    print(w$message)
                }
            }
        )
