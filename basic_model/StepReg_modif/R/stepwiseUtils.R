...
getModel <- function(data, type, intercept, x_name, y_name, weight, method = c("efron", "breslow", "exact"), n_ite = 100) {
  ...
  if (type == "linear") {
    ...
    model_raw <- glm(formula_raw, data = data, family = "binomial", control = glm.control(epsilon = 1e-8, maxit = n_ite, trace = FALSE))
...
getInitialSubSet <- function(data, type, metric, y_name, intercept, include, weight, test_method, sigma_value) {
  ...
  if (length(include) != 0) {
    initial_process_table <- data.frame(
      NumberOfVariables = numeric(),
      metric = numeric(),
...
getFinalSubSet <- function(data, type, metric, x_notin_model, initial_process_table, y_name, include, weight, intercept, best_n = Inf, test_method, sigma_value) {
  ...
  for (nv in 1:length(x_notin_model)) {
    ...
    x_name_list <- lapply(x_test_list, function(x) {
      c(intercept, include, x)
    })
    x_fit_list <- lapply(x_name_list, function(x) {
      getModel(data = data, type = type, intercept = intercept, x_name = x, y_name = y_name, weight = weight, method = test_method)
    })

    if (metric == "SL") {
      if (type == "cox") {
        pic_set <- sapply(x_fit_list, function(x) {
          x$score
        })
      } else {
        fit_reduce <- switch(type,
        ...
        f_pic_vec <- sapply(x_fit_list, function(x) {
          getAnovaStat(add_or_remove = "add", include = include, fit_reduced = fit_reduce, fit_full = x, type = type, test_method = test_method)
        })
        pic_set <- f_pic_vec[1, ]
      }
    } else {
      pic_set <- sapply(x_fit_list, function(x) {
        getModelFitStat(metric, x, type, sigma_value)
      })
    }
...
initialProcessTable <- function(metric) {
  sub_init_process_table <- data.frame(
    Step = numeric(),
...
getCandStepModel <- function(add_or_remove, data, type, metric, weight, y_name, x_in_model, x_notin_model, intercept, include, test_method, sigma_value) {
  ...
  if (BREAK == FALSE) {
    if (add_or_remove == "add") {
      ...
      x_name_list <- lapply(x_test_list, function(x) {
        setdiff(x_in_model, x)
      })
    }
    x_fit_list <- lapply(x_name_list, function(x) {
      getModel(data = data, type = type, intercept = intercept, x_name = c(include, x), y_name = y_name, weight = weight, method = test_method)
    })
    ...
    if (metric == "SL") {
      if (type != "linear" & add_or_remove == "remove") {
        ...
        f_pic_vec <- sapply(x_fit_list, function(x) {
          getAnovaStat(add_or_remove = add_or_remove, include = include, fit_reduced = x, fit_full = fit_x_in_model, type = type, test_method = test_method)
        })
        pic_set <- f_pic_vec[2, ]
        f_set <- f_pic_vec[1, ]
...
getTable3ProcessSummary <- function(data, type, strategy, metric, sle, sls, weight, x_name, y_name, intercept, include, best_n, test_method, sigma_value, num_digits) {
  ...
  for (stra in strategy) {
    for (met in metric) {
      ...
      table3[[paste0("Summary of selection process under ", stra, " with ", met, collapse = "")]] <- table3_process_table %>%
        mutate_if(is.numeric, round, num_digits) %>%
        mutate_if(is.numeric, as.character) # to keep digits as we expected, convert numeric to character for html output.
    ...
    for (met in metric) {
      if (!(stra == "subset" & met == "SL")) {
        ...
        for (i in names(table4_coef_model)) {
          table4[[paste0("Summary of coefficients for the selected model with ", i, " under ", stra, " and ", met, sep = " ")]] <- table4_coef_model[[i]] %>%
            mutate_if(is.numeric, round, num_digits) %>%
