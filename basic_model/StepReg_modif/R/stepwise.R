...
#'
#' @param n_ite (numeric(integer)) The number of iterations for optimization of glm in logistic model. Default is 100.
#'
#' @references
#'
...
stepwise <- function(formula,
  ...
  model_raw <- getModel(data, type = type, intercept = intercept, x_name_orig, y_name, weight = weight, method = test_method_cox, n_ite = n_ite)
  if (type != "cox") {
