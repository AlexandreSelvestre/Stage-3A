...
create_mean_x_J <- function(K, L, li_d, beta, classe_num, mu_amplif_bloc, sigma_amplif_bloc, sigma_amplif_global) {
    ...
    amplif_factor_bloc <- runif(L, mu_amplif_bloc, sigma_amplif_bloc)
...
create_mean_tab <- function(K, J, M, beta, classe_num, sigma_amplif_global) {
    ...
    val_class <- ifelse(classe_num == 1, 1, -1)
    return(sign_beta_tab * amplif_factor_tab * val_class)
}


create_Sigma_bloc <- function(K, d_l) {
    mat <- matrix(runif(K * d_l, 1, 1), nrow = K * d_l, ncol = K * d_l)
    Sigma_bloc <- mat %*% t(mat)
    Sigma_bloc <- Sigma_bloc / norm(as.matrix(Sigma_bloc), type = "2")
    return(Sigma_bloc)
}

generate_li_param <- function(n, J, K, L, M, li_d, R, beta, classe_num, mu_amplif_bloc, sigma_amplif_bloc, sigma_amplif_global, sigma_amplif_K) {
    li_mean_x_K <- lapply(1:R, function(r) {
        create_mean_x_K(K, L, sigma_amplif_K)
    })
    li_mean_x_J <- lapply(1:R, function(r) {
        create_mean_x_J(K, L, li_d, beta, classe_num, mu_amplif_bloc, sigma_amplif_bloc, sigma_amplif_global)
    })
...
create_param_explanatory <- function(K, L, J, M, li_d, R, n, beta, prop, mu_amplif_bloc, sigma_amplif_bloc, sigma_amplif_global, sigma_noise, sigma_amplif_K) {
    ...
    n_1 <- round(n * prop)
    n_0 <- n - n_1
    li_param_0 <- generate_li_param(n_0, J, K, L, M, li_d, R, beta, 0, mu_amplif_bloc, sigma_amplif_bloc, sigma_amplif_global, sigma_amplif_K)
    li_param_1 <- generate_li_param(n_1, J, K, L, M, li_d, R, beta, 1, mu_amplif_bloc, sigma_amplif_bloc, sigma_amplif_global, sigma_amplif_K)
...
apply_logistic <- function(mat_tot, beta, beta_0, index_variable, is_binary) {
    ...
    # write_xlsx(as.data.frame(df_renorm), "..\\data\\data_test.xlsx")
...
extract_all <- function(config_extrac) {
    ...
    if (config_extrac$resample_explain) {
        mu_amplif_bloc <- config_extrac$create_var$mu_amplif_bloc
        sigma_amplif_bloc <- config_extrac$create_var$sigma_amplif_bloc
        ...
        mat_tot <- create_param_explanatory(K, L, J, M, li_d, R, n, beta, prop, mu_amplif_bloc, sigma_amplif_bloc, sigma_amplif_global, sigma_noise, sigma_amplif_K)
    } else {
