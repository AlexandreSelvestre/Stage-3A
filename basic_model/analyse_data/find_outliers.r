...
variables <- data_used[, setdiff(colnames(data_used), c("X", "patient_num", "keys"))]
variables$classe_name <- 2 - as.integer(as.factor(variables$classe_name))
variables <- variables[order(variables$classe_name), ]
variables[, setdiff(colnames(variables), c("classe_name"))] <- renormalize_in_model_fit(variables[, setdiff(colnames(variables), c("classe_name"))])$x
...
var_to_clust <- variables[, setdiff(colnames(variables), c("classe_name"))]
...
if (EM) {
    ...
    li_clust <- NbClust(data = var_to_clust, distance = "euclidean", min.nc = 4, max.nc = 20, method = "ward.D2", index = "kl")
    ...
    print(variables$classe_name) # 1  = CCK, 0 = CHC
}
...
pca_data <- data.frame(PC1 = data_pca[, 1], PC2 = data_pca[, 2], Classe = variables$classe_name)
...
ggsave("./clusters.png", image, width = 8, height = 6, dpi = 300)
