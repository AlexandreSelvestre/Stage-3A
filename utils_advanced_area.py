...
if __name__ == '__main__':
    ...
    # dic_areas = get_profile_brut(image, mask)
    # depths_x, areas = interpolate_initial_space(dic_areas, n_points = 1000)
    # interpolator, min_depth, max_depth, min_area, max_area = interpolate_reciprocal(depths_x, areas)
    # li_areas_searched = np.linspace(min_area, max_area, 1000)
    # li_depths_found = interpolator.__call__(li_areas_searched)
    # plt.subplot(1,2,1)
    # plt.plot(li_areas_searched, li_depths_found)
    # plt.plot(areas, 1.02*depths_x)
    # plt.subplot(1,2,2)
    # plt.plot(depths_x, areas) 
...
#Si on a bien interpôlé, on peut recaler tout le monde avvce le grand produit d'intégrales et recaler les courbes à précision voulue.
#afficher les courbes recalées.
#Puis définir à partir de là les profondeurs et les slices.. On a besoin de chaque image à un tps donné pour défnir les slices de chacun; C'est propre et beau.
    ...
    #on sait qu'avant et après les indices de indices for interpolation c'est nul
