...
def cercle_image(slice_img, slice_mask, name="default_name", num="_", show=False):
    slice_good = sitk.GetArrayFromImage(slice_img)
    mask_slice_good = sitk.GetArrayFromImage(slice_mask)
    mask_slice_good = mask_slice_good.astype(np.uint8)
    mask_dilation_first = cv2.dilate(mask_slice_good, np.ones(
        (3, 3), np.uint8), iterations=5)  # 10 et 18
    mask_dilation_second = cv2.dilate(
        mask_slice_good, np.ones((3, 3), np.uint8), iterations=15)
    ...
    # + mask_dilation_4 - mask_dilation_3
    ...
    quantile_low_lum = 0.5 * np.quantile(slice_good.flatten(), 0.9) + 0.5 * np.min(
        slice_good.flatten())  # pas être dans le noir complet
    ...
    if show:
        ...
        plt.imshow(variance)
        plt.subplot(1, 3, 3)
        plt.hist(variance[mask_cercle == 1].flatten(),
                 bins=bins, edgecolor='black')
        plt.title("Histogramme des valeurs des pixels")
        plt.xlabel("Valeur des pixels")
        plt.ylabel("Fréquence")
        plt.show()
        # slice_image, slice_mask,other_mask = None, num = 1, max_num = 1, legend = "No legend"
        mask_superpose_simple(slice_good, mask_slice_good,
                              other_mask=mask_cercle)
        mask_superpose_simple(slice_good, mask_slice_good,
                              other_mask=cercle_lumineux)
...
def compute_score(i, components, slice_good, mask_slice_good, mask_cercle_lumineux, intens_standard, coeff=0):
    ...
    intens_median_tumor = np.quantile(
        slice_good[mask_slice_good == 1].flatten(), 0.5)
    size_cercle = np.sum(mask_cercle_lumineux)
    size_relat_component_i = np.sum(component_i)/size_cercle
    intens_compnent_i = np.quantile(
        slice_good[component_i == 1].flatten(), 0.5)
    ecart_intens_relat_component_i = np.abs(
        intens_compnent_i - intens_median_tumor) / intens_standard
...
def compute_standard(components, num_comp, slice_good, mask_slice_good):
    ...
    for i in range(1, num_comp+1):
        max_intens = max(max_intens, np.quantile(
            slice_good[components == i].flatten(), 0.5))
    return max_intens


def cercle_image_compos_connexe(slice_img, slice_mask, name="default_name", num="_", show=False):
    mask_cercle_lumineux, slice_good, mask_slice_good, mask_cercle = cercle_image(
        slice_img, slice_mask, name=name, num=num, show=False)
    # l'indexation des composantes commence à 1
    components, num_comp = nd.label(mask_cercle_lumineux)
    intens_standard = compute_standard(
        components, num_comp, slice_good, mask_slice_good)
    dict_sizes = {str(i): compute_score(i, components, slice_good, mask_slice_good,
                                        mask_cercle_lumineux, intens_standard) for i in range(1, num_comp + 1)}
    dict_sizes = dict(
        sorted(dict_sizes.items(), key=lambda item: item[1], reverse=True))
    size_to_cover = min(np.sum(mask_cercle) * 0.05,
                        np.sum(mask_cercle_lumineux) * 0.5)
    covered_size = 0
    ...
    if (len(li_keys_compos_connexe) > 0):
        ...
        print("echec mask vide image", name, "slice", num)
    ...
    if show:
        mask_superpose_simple(slice_good, mask_slice_good,
                              other_mask=mask_cercle_lumineux)
        mask_superpose_simple(slice_good, mask_slice_good,
                              other_mask=mask_cercle_compos_connexe)
...
if __name__ == "__main__":
    ...
    slice_image = sitk.ReadImage(ls_image[0])[:, :, 36]
    slice_mask = sitk.ReadImage(ls_image[0].replace(
        '_NAT', '').replace('.nii', '_masked.nii'))[:, :, 36]
    cercle_image_compos_connexe(slice_image, slice_mask, show=True)
    plt.show()
