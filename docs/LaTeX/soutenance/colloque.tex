...
\begin{frame}
    ...
    \end{align}
    \onslide<2->{
    \textbf{Strategy}: dilate $\bm{\beta}_{\ell,r}^J$ by $\lVert \bm{\beta}_{\ell,r}^K\rVert_1$ and $x_{j,k}^\ell$ by $\lVert \bm{\beta}_{\ell,r}^K\rVert_1^{-1}$, so
    $$\mathbf{x}^T\bm{\beta} =  \sum\limits_{j,\ell,r} \left(\sum\limits_{k}  x_{j,k}^\ell(\beta_{\ell,r}^{K})_k \, \right)(\beta_{\ell,r}^{J})_j$$
     does not change but 
    ...
    \onslide<3->{
    \noindent Then, do logistic lasso regression with $\mathbf{x} = (x_{ijk}^l\lVert \bm{\beta}_{\ell,r}^K\rVert_1^{-1})$ and $\lambda$ penalty coefficient to find $\left((\beta_{\ell,r}^J)_j\lVert \bm{\beta}_{\ell,r}^K \rVert_1 \right)$}
